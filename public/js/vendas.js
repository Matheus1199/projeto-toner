...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const clienteSelecionadoHidden =
      ...
      try {
        ...
        tabelaPesquisa.innerHTML += `
            <tr class="border-t bg-gray-50 font-semibold">
                ...
                <td class="py-2 px-3 text-green-600">R$ ${totalGeral.toFixed(
                  2
                )}</td>
            </tr>
        `;
      ...
      clienteDebounce = setTimeout(async () => {
        ...
        try {
          const resp = await fetch(
            `/clientes/pesquisar?nome=${encodeURIComponent(termo)}`
          );
          ...
          const lista = Array.isArray(json)
            ? json
            : json.clientes || (json.cliente ? [json.cliente] : []);
          if (!lista || !lista.length) {
            resultadoClientes.innerHTML =
              "<p class='text-gray-600 p-2'>Nenhum cliente encontrado.</p>";
            return;
          }
          lista.forEach((cli) => {
            const card = document.createElement("div");
            card.className =
              "bg-white p-3 rounded-xl mb-2 border shadow-sm cursor-pointer hover:bg-gray-50";
            card.innerHTML = `
            ...
            card.addEventListener("click", () => {
              ...
              clienteSelecionadoHidden.value =
                cli.Id_Cliente || cli.Id_cliente || cli.id;
              resultadoClientes.classList.add("hidden");
            });
  ...
  document.addEventListener("click", (e) => {
    if (
      !inputPesquisaCliente.contains(e.target) &&
  ...
  let tonerDebounce;
  inputPesquisaToner &&
    inputPesquisaToner.addEventListener("input", () => {
      ...
      tonerDebounce = setTimeout(async () => {
        ...
        try {
          // 🔥 lista de Id_ItemCompra já usados no carrinho
          const idsCarrinho = carrinho.map((i) => i.Id_ItemCompra);

          const consumidos = {};

          carrinho.forEach((item) => {
            if (!consumidos[item.Id_ItemCompra])
              consumidos[item.Id_ItemCompra] = 0;
            consumidos[item.Id_ItemCompra] += item.Quantidade;
          });

          const resp = await fetch("/estoque/buscar", {
            method: "POST",
          ...
          if (!dados || !dados.length) {
            listaSugestoesToner.innerHTML = `<div class="p-2 text-gray-600">Nenhum lote disponível</div>`;
          ...
          dados.forEach((lote) => {
            ...
            div.innerHTML = `
                    <div class="font-semibold">${lote.Marca} - ${
              lote.Modelo
            } (${lote.Tipo || ""})</div>
                    <div class="text-sm text-gray-600">Compra #${
                      lote.Cod_Compra
                    } — Saldo: ${lote.Saldo}</div>
                    <div class="text-sm text-gray-600">Valor compra: R$ ${Number(
                      lote.Valor_Compra
                    ).toFixed(2)}</div>
                `;
            div.addEventListener("click", () => {
              loteSelecionado = lote;
              inputPesquisaToner.value = `${lote.Marca} - ${lote.Modelo} (${
                lote.Tipo || ""
              }) - Saldo: ${lote.Saldo}`;
  ...
  document.addEventListener("click", (e) => {
    if (
      !inputPesquisaToner.contains(e.target) &&
      !listaSugestoesToner.contains(e.target)
    ) {
      listaSugestoesToner.classList.add("hidden");
    }
  });
  ...
  btnAdicionarItem &&
    btnAdicionarItem.addEventListener("click", () => {
      ...
      if (qtd > loteSelecionado.Saldo)
        return alert(`Saldo insuficiente: ${loteSelecionado.Saldo}`);
  ...
  function atualizarCarrinho() {
    ...
    carrinho.forEach((it, idx) => {
      tbodyCarrinho.insertAdjacentHTML(
        "beforeend",
        `
                <tr>
                    <td class="py-2 px-3">${it.Nome_Produto}</td>
                    <td class="py-2 px-3 text-center">${it.Quantidade}</td>
                    <td class="py-2 px-3 text-center">${fmtBRL(
                      it.Valor_Venda
                    )}</td>
                    <td class="py-2 px-3 text-center font-semibold">${fmtBRL(
                      it.Subtotal
                    )}</td>
  ...
  function atualizarTabelaFinanceiro() {
    ...
    listaFinanceiro.forEach((f, idx) => {
      tbodyFinanceiro.insertAdjacentHTML(
        "beforeend",
        `
                <tr>
