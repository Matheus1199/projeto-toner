...
app.get("/pagrec/pendentes", async (req, res) => {
    try {
        const result = await pool.request().query(`
            SELECT
                P.Id_Lancamento,
...
app.get("/pagrec/listar", async (req, res) => {
    try {
        const result = await pool.request().query(`
            SELECT 
                Id_Lancamento,
...
app.post("/pagrec/salvar", async (req, res) => {
    const {
        Id_Lancamento,
        Tipo,
        Operacao,
    ...
    try {
        if (!Id_Lancamento) {
            ...
            await pool.request()
                ...
                .query(`
                    INSERT INTO Tbl_PagRec (Tipo, Operacao, Valor, Data_Vencimento, Obs, Baixa)
                    VALUES (@Tipo, @Operacao, @Valor, @Data_Vencimento, @Obs, 0)
                `);
...
app.get("/pagrec/buscar/:id", async (req, res) => {
    const { id } = req.params;
...
app.delete("/pagrec/excluir/:id", async (req, res) => {
    const { id } = req.params;
